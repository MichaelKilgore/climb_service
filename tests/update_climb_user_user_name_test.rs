...
/*
 creates a user, changes their name to poopyjr,
 then changes their name back to what it was previously
...
fn test_update_climb_user_user_name_fails_because_user_name_already_exists() {
    ...
    if let Some(user_name) = actual_json.get("user_name").and_then(Value::as_str) {
        if let Some(_user_id) = actual_json.get("climb_user_id").and_then(Value::as_i64) {
